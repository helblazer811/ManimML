...
class Convolutional2DToTransConv2D(ConnectiveLayer, ThreeDLayer):
    ...
    def make_forward_pass_animation(self, layer_args={}, run_time=1.5, **kwargs):
        ...
        #     self.input_layer.feature_maps[0].get_corners_dict()["top_left"],
        #     submobject_to_align=gridded_rectangle.get_corners_dict()["top_left"],
        #     buff=0.0,
        # )
        gridded_rectangle_group.move_to(
            self.input_layer.feature_maps[0].get_center(),
        )


        transform_ans = []
        for i in range(len(self.input_layer.feature_maps.submobjects)):



            transform_previous = ReplacementTransform(
                self.input_layer.feature_maps.submobjects[i],#.exterior_rectangle,
                # feature_map.shift(prev_layer.feature_maps.submobjects[i].exterior_rectangle.get_center()),
                gridded_rectangle_group,
                run_time=1,
            )
            transform_ans.append(transform_previous)
            transform_ans.append(Wait(10))

        return Succession(AnimationGroup(*transform_ans), Wait(1))
